...
int send_highsgm(terminal_t *p, int sgm_n, int ter_n) {/*現在接続している端末へセグメントsgmを放送する */
	printf("\nsend_sgm_by_multicast:%d USER:%d\n", sgm_n,ter_n);
	int i;
	for (i = 0; i < ter_n; i++, p++) {
		if ((*p).break_flag != 1) {
...
int send_sgm_p2p3(terminal_t *p, int num){ /*接続ユーザ内で最も古いセグメントを要求しているユーザにP2Pでセグメント送信*/
	...
	if(req_user == ter_n){	//動画が配信されていない高課金ユーザがいないなら低課金ユーザにスケジューリングを行う
...
double play_postponement_hightime(terminal_t *p) {         /*余裕時間を計算する*/
	if ((*p).break_flag == 1 || (*p).finish_flag != 0) {
		return 10000;
	}
...
double set_c_return_highsgm_num(terminal_t *p, int ter_n) {/*最小余裕時間を持つユーザを探し，そのユーザの要求しているセグメントを返す*/
	int i;
	int min_PPT_sgm = 0;
	double eps = 1e-10;
	double p_i;
...
bool conplete_sgm(terminal_t *p, int ter_n) {   /*すべてのユーザがセグメントを受信完了できたか判定する*/
	...
	for (i = 1; i <= ter_n; i++, p++) {
		...
		if ((*p).break_flag != 1) {
			...
			if ((*p).billing_flag == 2) {	//全高課金ユーザが高画質セグメントを受信できるまで終了しない
...
double caluclate_wait_t(terminal_t *p, int ter_n, double before_wait_time[]) {/*平均中断時間の計算*/
	...
	k2_avarage = kakin2_wait_time / k2_user; /*重課金ユーザの平均待ち時間*/
	k_avarage = kakin_wait_time / k_user;	/*課金ユーザの平均待ち時間*/
